...
summarise_vax_history = function(data) {
  data <- data %>%
    summarise(
      ...
      Dose_5 = plyr::round_any(sum(n_vax==5), rounding_threshold),
      Dose_6 = plyr::round_any(sum(n_vax==6), rounding_threshold),
      Dose_7 = plyr::round_any(sum(n_vax==7), rounding_threshold),
...
tab_combined_clean <- tab_combined %>%
  mutate(
    ...
    `5 dose` = paste0(Dose_5," (",round(Dose_5/N,1),"%)"),
    `6 dose` = paste0(Dose_5," (",round(Dose_6/N,1),"%)"),
    `7 dose` = paste0(Dose_5," (",round(Dose_7/N,1),"%)"),
  ...
  select(Population, Subgroup, Level, N, 
         `0 dose`, `1 dose`, `2 dose`, `3 dose`, `4 dose`, `5 dose`, `6 dose`, `7 dose`,
